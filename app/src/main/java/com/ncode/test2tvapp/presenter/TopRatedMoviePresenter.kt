...
import com.ncode.test2tvapp.model.TopRatedResult
import com.ncode.test2tvapp.retrofit.RetroClient

class TopRatedMoviePresenter: Presenter() {
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder?, item: Any?) {
        ...
        Glide.with(viewHolder.view.context).load(RetroClient.POSTER_URL + trendingList.poster_path)
            .centerCrop().into(cardView.mainImageView)
        cardView.titleText = trendingList.original_title
        cardView.contentText = trendingList.vote_average.toString()
    }
